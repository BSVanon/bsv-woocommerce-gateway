...
function BWWC__render_dashboard_widget()
{
    ...
    if (!class_exists('WooCommerce')) {
        echo '<p>' . esc_html__('WooCommerce is not active.', 'bitcoin-sv-payments-for-woocommerce') . '</p>';
        return;
    }
    ...
    if ($is_valid) {
        ...
        echo '<strong style="color: #46b450;">✓ ' . esc_html__('Gateway Operational', 'bitcoin-sv-payments-for-woocommerce') . '</strong>';
        echo '</div>';
    } else {
        echo '<div style="padding: 10px; background: #fff3cd; border-left: 4px solid #ffc107;">';
        echo '<strong style="color: #856404;">⚠ ' . esc_html__('Gateway Issue', 'bitcoin-sv-payments-for-woocommerce') . '</strong><br>';
    ...
    if ($store_currency != 'BTC') {
        ...
        echo '<strong>' . esc_html__('Exchange Rate:', 'bitcoin-sv-payments-for-woocommerce') . '</strong> ';
        if ($exchange_rate) {
            echo '1 BSV = ' . number_format($exchange_rate, 2) . ' ' . esc_html($store_currency);
        } else {
            echo '<span style="color: #dc3545;">' . esc_html__('Unable to fetch', 'bitcoin-sv-payments-for-woocommerce') . '</span>';
        }
        echo '</div>';
    }
    
    // Recent BSV orders
    echo '<h4 style="margin: 15px 0 10px 0;">' . esc_html__('Recent BSV Orders', 'bitcoin-sv-payments-for-woocommerce') . '</h4>';
    ...
    if (empty($orders)) {
        echo '<p style="color: #666; font-style: italic;">' . esc_html__('No BSV orders yet.', 'bitcoin-sv-payments-for-woocommerce') . '</p>';
    } else {
        echo '<table style="width: 100%; font-size: 12px;">';
        echo '<thead><tr style="border-bottom: 1px solid #ddd;">';
        echo '<th style="text-align: left; padding: 5px;">' . esc_html__('Order', 'bitcoin-sv-payments-for-woocommerce') . '</th>';
        echo '<th style="text-align: left; padding: 5px;">' . esc_html__('Status', 'bitcoin-sv-payments-for-woocommerce') . '</th>';
        echo '<th style="text-align: right; padding: 5px;">' . esc_html__('Total', 'bitcoin-sv-payments-for-woocommerce') . '</th>';
        echo '</tr></thead><tbody>';
    ...
    echo '<a href="' . esc_url(admin_url('admin.php?page=BWWC-settings')) . '" class="button button-small">' . esc_html__('Gateway Settings', 'bitcoin-sv-payments-for-woocommerce') . '</a> ';
    echo '<a href="' . esc_url(admin_url('admin.php?page=wc-orders&payment_method=bitcoin')) . '" class="button button-small">' . esc_html__('View All BSV Orders', 'bitcoin-sv-payments-for-woocommerce') . '</a>';
    echo '</p>';
}
