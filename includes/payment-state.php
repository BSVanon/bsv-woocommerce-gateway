...
function BWWC__handle_payment_state_change($order_id, $old_state, $new_state)
{
    ...
    switch ($new_state) {
        case BWWC_PAYMENT_STATE_DETECTED:
            ...
            if (!empty($settings['autocomplete_paid_orders'])) {
                $order->update_status('processing', __('Payment detected on blockchain', 'sendbsv-bsv-payments-for-woocommerce'));
            }
            break;
        ...
        case BWWC_PAYMENT_STATE_VERIFIED:
            ...
            // Complete the order
            if (!empty($settings['autocomplete_paid_orders'])) {
                $order->update_status('completed', __('Payment verified with required confirmations', 'sendbsv-bsv-payments-for-woocommerce'));
            } else {
                $order->update_status('processing', __('Payment verified with required confirmations', 'sendbsv-bsv-payments-for-woocommerce'));
            }
        ...
        case BWWC_PAYMENT_STATE_EXPIRED:
            ...
            if ($old_state === BWWC_PAYMENT_STATE_WAITING) {
                ...
                if (!empty($settings['delete_expired_unpaid_orders'])) {
                    $order->update_status('cancelled', __('Payment window expired', 'sendbsv-bsv-payments-for-woocommerce'));
                }
            }
            break;
        ...
        case BWWC_PAYMENT_STATE_UNDERPAID:
            ...
            $order->add_order_note(__('Partial payment received. Waiting for remaining amount.', 'sendbsv-bsv-payments-for-woocommerce'));
            break;
            
        case BWWC_PAYMENT_STATE_OVERPAID:
            // Overpayment received
            $order->add_order_note(__('Overpayment received. Please contact customer to arrange refund.', 'sendbsv-bsv-payments-for-woocommerce'));
            break;
            
        case BWWC_PAYMENT_STATE_CONFLICT:
            // Conflicting transactions detected
            $order->update_status('on-hold', __('Payment conflict detected. Manual review required.', 'sendbsv-bsv-payments-for-woocommerce'));
            break;
    }
    ...
    if ($old_state === BWWC_PAYMENT_STATE_EXPIRED && 
        ...
        $order->update_status('on-hold', __('Late payment received after expiry. Manual review required.', 'sendbsv-bsv-payments-for-woocommerce'));
