...
function BWWC__serve_bip270_invoice() {
	...
	if ( ! is_ssl() && ! defined( 'BWWC_ALLOW_HTTP_INVOICE' ) ) {
		BWWC__log_bip270_invoice(
			'Rejected invoice request: HTTPS required',
			array(
	...
	if ( ! $order_id || ! $order_key || ! $signature ) {
		BWWC__log_bip270_invoice(
			'Invoice request missing parameters',
			array(
				'orderId' => $order_id,
				'hasKey'  => $order_key !== '' ? 'yes' : 'no',
				'hasSig'  => $signature !== '' ? 'yes' : 'no',
	...
	if ( ! $order ) {
		BWWC__log_bip270_invoice(
			'Invoice request failed: order not found',
			array(
				'orderId' => $order_id,
			)
		);
	...
	if ( $order->get_order_key() !== $order_key ) {
		BWWC__log_bip270_invoice(
			'Invoice request failed: invalid order key',
			array(
				'orderId' => $order_id,
			)
		);
	...
	if ( ! hash_equals( $expected_sig, $signature ) ) {
		BWWC__log_bip270_invoice(
			'Invoice request failed: invalid signature',
			array(
				'orderId' => $order_id,
			)
		);
	...
	if ( ! in_array( $payment_state, array( 'waiting', 'underpaid', 'pending', 'detected' ) ) ) {
		BWWC__log_bip270_invoice(
			'Invoice request rejected due to order state',
			array(
				'orderId' => $order_id,
				'state'   => $payment_state,
			)
	...
	if ( ! $bsv_address || ! $bsv_amount || ! $expected_sats ) {
		BWWC__log_bip270_invoice(
			'Invoice request failed: missing payment details',
			array(
				'orderId'         => $order_id,
	...
	if ( $expires_at && time() > $expires_at ) {
		BWWC__log_bip270_invoice(
			'Invoice request failed: invoice expired',
			array(
				'orderId'   => $order_id,
	...
	if ( ! $locking_script ) {
		BWWC__log_bip270_invoice(
			'Invoice request failed: locking script generation failed',
			array(
				'orderId' => $order_id,
			)
		);
...
function BWWC__serve_receipt_download() {
	...
	if ( ! $order ) {
		BWWC__log_bip270_invoice(
			'Receipt download failed: order not found',
			array(
				'orderId' => $order_id,
			)
		);
	...
	if ( $order->get_order_key() !== $order_key ) {
		BWWC__log_bip270_invoice(
			'Receipt download failed: invalid order key',
			array(
				'orderId' => $order_id,
			)
		);
	...
	if ( ! hash_equals( $expected_sig, $signature ) ) {
		BWWC__log_bip270_invoice(
			'Receipt download failed: invalid signature',
			array(
				'orderId' => $order_id,
			)
		);
	...
	if ( ! $raw_tx && ! $beef ) {
		BWWC__log_bip270_invoice(
			'Receipt download failed: no receipt data',
			array(
				'orderId' => $order_id,
			)
		);
