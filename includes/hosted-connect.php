...
function BWWC__hosted_connect_init() {
	...
	add_action( 'admin_init', 'BWWC__maybe_handle_hosted_connect_callback' );
}
...
/**
 * Build callback URL used by guided connect flow.
 *
 * @param string $state_token State token generated by Woo.
 * @return string
 */
function BWWC__get_hosted_connect_callback_url( $state_token ) {
	return add_query_arg(
		array(
			'page'                 => 'wc-settings',
			'tab'                  => 'checkout',
			'section'              => 'bsv_gateway',
			'bwwc_hosted_callback' => '1',
			'bwwc_hosted_state'    => $state_token,
		),
		admin_url( 'admin.php' )
	);
}
...
/**
 * Build deterministic transient key for guided connect state.
 *
 * @param string $state_token State token.
 * @return string
 */
function BWWC__get_hosted_connect_transient_key( $state_token ) {
	return 'bwwc_hosted_connect_' . md5( (string) $state_token );
}
...
/**
 * Start hosted connection flow (guided redirect + one-time grant exchange).
 */
function BWWC__ajax_hosted_connect_start() {
	...
	$callback_url  = BWWC__get_hosted_connect_callback_url( $state_token );
	$transient_key = BWWC__get_hosted_connect_transient_key( $state_token );
	set_transient(
		$transient_key,
		array(
	...
	$response = wp_remote_post(
		$api_base_url . '/v1/connectors/woocommerce/connect/start',
		array(
			'timeout' => 30,
			'headers' => array(
				'content-type' => 'application/json',
	...
	if ( is_wp_error( $response ) ) {
		wp_send_json_error(
			array(
				'message' => 'Unable to start guided connect: ' . $response->get_error_message(),
			)
		);
	}
	...
	if ( $status < 200 || $status >= 300 ) {
		$error_message = isset( $data['error'] ) ? (string) $data['error'] : 'Hosted connect start failed';
		wp_send_json_error(
			array(
	...
	$connect_url = isset( $data['connect_url'] ) ? trim( (string) $data['connect_url'] ) : '';
	if ( '' === $connect_url ) {
		wp_send_json_error(
			array(
				'message' => 'Hosted connect start succeeded but no connect URL was returned.',
			)
		);
	}
	...
	wp_send_json_success(
		array(
			...
			'message'     => 'Continue setup in SendBSV Invoicing and approve connection to return here automatically.',
		)
	);
}

/**
 * Handle guided hosted connect callback and exchange one-time grant for connector key.
 */
function BWWC__maybe_handle_hosted_connect_callback() {
	if ( ! is_admin() ) {
		return;
	}
	if ( ! isset( $_GET['bwwc_hosted_callback'] ) || '1' !== sanitize_text_field( wp_unslash( $_GET['bwwc_hosted_callback'] ) ) ) {
		return;
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( 'Unauthorized', 403 );
	...
	$state_token = isset( $_GET['bwwc_hosted_state'] ) ? sanitize_text_field( wp_unslash( $_GET['bwwc_hosted_state'] ) ) : '';
	$grant_token = isset( $_GET['bwwc_hosted_grant'] ) ? sanitize_text_field( wp_unslash( $_GET['bwwc_hosted_grant'] ) ) : '';
	...
	if ( '' === $state_token || '' === $grant_token ) {
		wp_safe_redirect(
			add_query_arg(
				array(
					'bwwc_hosted_connect_error' => 'Missing connect callback parameters.',
				),
				$settings_url
			)
		);
	...
	if ( ! is_array( $state_data ) ) {
		wp_safe_redirect(
			add_query_arg(
				array(
					'bwwc_hosted_connect_error' => 'Connect state expired. Start Hosted connect again.',
				),
				$settings_url
			)
		);
	...
	$user_id = isset( $state_data['user_id'] ) ? intval( $state_data['user_id'] ) : 0;
	if ( $user_id > 0 && $user_id !== get_current_user_id() ) {
		wp_safe_redirect(
			add_query_arg(
				array(
					'bwwc_hosted_connect_error' => 'Connect state belongs to a different admin session.',
				),
				$settings_url
			)
		);
	...
	$api_base_url  = ! empty( $bwwc_settings['hosted_api_base_url'] )
		? rtrim( $bwwc_settings['hosted_api_base_url'], '/' )
		: 'https://sendbsv-invoicing.proud-mode-7a3d.workers.dev';
	$callback_url  = isset( $state_data['callback_url'] ) ? (string) $state_data['callback_url'] : BWWC__get_hosted_connect_callback_url( $state_token );

	$response = wp_remote_post(
		$api_base_url . '/v1/connectors/woocommerce/connect/exchange',
		array(
			'timeout' => 30,
			'headers' => array(
				'content-type' => 'application/json',
	...
	if ( is_wp_error( $response ) ) {
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					'bwwc_hosted_connect_error' => 'Grant exchange failed: ' . $response->get_error_message(),
				),
				$settings_url
			)
		);
	...
	if ( $status < 200 || $status >= 300 ) {
		$error_message = isset( $data['error'] ) ? (string) $data['error'] : 'Grant exchange failed';
		delete_transient( $transient_key );
		wp_safe_redirect(
			add_query_arg(
				array(
					'bwwc_hosted_connect_error' => $error_message,
				),
				$settings_url
			)
		);
	...
	$connector_key = isset( $data['connector_key'] ) ? trim( (string) $data['connector_key'] ) : '';
	$merchant_id   = isset( $data['merchant_id'] ) ? trim( (string) $data['merchant_id'] ) : '';
	if ( '' === $connector_key ) {
		delete_transient( $transient_key );
		wp_safe_redirect(
			add_query_arg(
				array(
					'bwwc_hosted_connect_error' => 'Exchange succeeded but connector key was missing.',
				),
				$settings_url
			)
		);
	...
	$bwwc_settings['hosted_connection_ref']   = $merchant_id;
	...
	if ( empty( $test_result['success'] ) ) {
		$message = isset( $test_result['message'] ) ? (string) $test_result['message'] : 'Connected but Test Connection needs attention.';
		wp_safe_redirect(
			add_query_arg(
				array(
					'bwwc_hosted_connected' => '1',
					'bwwc_hosted_notice'    => $message,
				),
				$settings_url
			)
		);
...
function BWWC__hosted_connection_admin_notice() {
	if ( isset( $_GET['bwwc_hosted_connect_error'] ) ) {
		$message = sanitize_text_field( wp_unslash( $_GET['bwwc_hosted_connect_error'] ) );
		printf(
	...
	if ( ! isset( $_GET['bwwc_hosted_connected'] ) || '1' !== sanitize_text_field( wp_unslash( $_GET['bwwc_hosted_connected'] ) ) ) {
		return;
	}
	$message = esc_html__( 'Hosted Invoicing connected successfully.', 'bsvanon-bitcoin-sv-payments' );
	if ( isset( $_GET['bwwc_hosted_notice'] ) ) {
		$notice = sanitize_text_field( wp_unslash( $_GET['bwwc_hosted_notice'] ) );
		if ( '' !== $notice ) {
			$message .= ' ' . $notice;
		}
	}
