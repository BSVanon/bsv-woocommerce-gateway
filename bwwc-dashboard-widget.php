...
function BWWC__register_dashboard_widget()
{
    wp_add_dashboard_widget(
        'bwwc_gateway_status',
        __('Bitcoin SV Gateway Status', 'bitcoin-sv-woocommerce'),
...
function BWWC__render_dashboard_widget()
{
    ...
    if (!class_exists('WooCommerce')) {
        echo '<p>' . __('WooCommerce is not active.', 'bitcoin-sv-woocommerce') . '</p>';
        return;
    }
    ...
    if ($is_valid) {
        ...
        echo '<strong style="color: #46b450;">✓ ' . __('Gateway Operational', 'bitcoin-sv-woocommerce') . '</strong>';
        echo '</div>';
    } else {
        echo '<div style="padding: 10px; background: #fff3cd; border-left: 4px solid #ffc107;">';
        echo '<strong style="color: #856404;">⚠ ' . __('Gateway Issue', 'bitcoin-sv-woocommerce') . '</strong><br>';
    ...
    if ($store_currency != 'BTC') {
        ...
        echo '<strong>' . __('Exchange Rate:', 'bitcoin-sv-woocommerce') . '</strong> ';
        if ($exchange_rate) {
            echo '1 BSV = ' . number_format($exchange_rate, 2) . ' ' . esc_html($store_currency);
        } else {
            echo '<span style="color: #dc3545;">' . __('Unable to fetch', 'bitcoin-sv-woocommerce') . '</span>';
        }
        echo '</div>';
    }
    
    // Recent BSV orders
    echo '<h4 style="margin: 15px 0 10px 0;">' . __('Recent BSV Orders', 'bitcoin-sv-woocommerce') . '</h4>';
    ...
    if (empty($orders)) {
        echo '<p style="color: #666; font-style: italic;">' . __('No BSV orders yet.', 'bitcoin-sv-woocommerce') . '</p>';
    } else {
        echo '<table style="width: 100%; font-size: 12px;">';
        echo '<thead><tr style="border-bottom: 1px solid #ddd;">';
        echo '<th style="text-align: left; padding: 5px;">' . __('Order', 'bitcoin-sv-woocommerce') . '</th>';
        echo '<th style="text-align: left; padding: 5px;">' . __('Status', 'bitcoin-sv-woocommerce') . '</th>';
        echo '<th style="text-align: right; padding: 5px;">' . __('Total', 'bitcoin-sv-woocommerce') . '</th>';
        echo '</tr></thead><tbody>';
    ...
    echo '<a href="' . admin_url('admin.php?page=BWWC-settings') . '" class="button button-small">' . __('Gateway Settings', 'bitcoin-sv-woocommerce') . '</a> ';
    echo '<a href="' . admin_url('admin.php?page=wc-orders&payment_method=bitcoin') . '" class="button button-small">' . __('View All BSV Orders', 'bitcoin-sv-woocommerce') . '</a>';
    echo '</p>';
}
